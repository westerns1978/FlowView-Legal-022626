...
/* FIX: Changed QwOpportunity to SalesOpportunity as defined in types.ts */
import { Contract, SalesOpportunity, TravelLogEntry, InventoryItem, MsPoint, ProcessingResult } from '../types';
import sampleData, { enterpriseExpenses as sampleExpenses } from './sampleData.ts';
import { enterpriseUsers } from './flowview-users';
...
export let qwOpportunityData: any[] = [];
...
/* FIX: Changed QwOpportunity to SalesOpportunity to match exported members of types.ts */
export const salesPipelineData: SalesOpportunity[] = sampleData.salesPipelineData as SalesOpportunity[];
...
export const mergePostgresJson = (jsonData: { [fileName: string]: { content: any, size: number } }): {
    ...
    const fileHandlers: { [key: string]: (data: any[], result: ProcessingResult) => void } = {
        ...
        'qw_opportunity': (d, r) => { qwOpportunityData = d; r.recordCount = d.length; },
...
const buildFoundationalData = () => {
    if (users.length === 0) {
        users = enterpriseUsers.map((pUser, index) => ({
             ...
             email: `${pUser.id}@flowview-lex.com`, role: 'Field Tech', hourly_rate: 85.0,
    ...
    if (inventoryData.length === 0) {
        inventoryData = [
            { id: 101, warehouse_id: 1, name: "Konica Minolta TN-324K Toner", quantity: 45, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
            { id: 102, warehouse_id: 1, name: "Ricoh IM C3000 Drum Unit", quantity: 12, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
            { id: 103, warehouse_id: 2, name: "HP LaserJet Fuser Kit (110V)", quantity: 8, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
            { id: 104, warehouse_id: 1, name: "Xerox Waste Toner Container", quantity: 22, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
            { id: 105, warehouse_id: 3, name: "Canon Transfer Belt Assembly", quantity: 5, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
            { id: 106, warehouse_id: 2, name: "Lexmark 501H Return Program Cartridge", quantity: 30, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
        ];
    }
...
export const foundationalData = {
    customers: [
        ...
        { id: 'CUST-BO', name: 'Bluegrass Ortho', industry: 'Healthcare', healthScore: 71 },
        ...
        { id: 'CUST-DLF', name: 'Downtown Law Firm', industry: 'Legal', healthScore: 85 },
        { id: 'CUST-LFUG', name: 'Lexington Fayette Urban County Gov', industry: 'Government', healthScore: 76 }
    ],
    technicians: [] as any[],
    serviceCatalog: [
        { id: 'SVC-AV', name: 'AV Installation', unitPrice: 1500 },
        { id: 'SVC-DC', name: 'Data Center Cabling', unitPrice: 2500 },
        { id: 'SVC-WIFI', name: 'WiFi Upgrade', unitPrice: 1200 },
    ]
};
