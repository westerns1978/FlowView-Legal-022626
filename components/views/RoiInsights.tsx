...
export const RoiInsights: React.FC<RoiInsightsProps> = ({ onOpenAiCommandCenter, appData }) => {
    ...
    const prompt = `Analyze these ROI trends. First call fix rate is ${roiData.firstCallFixRate} and average utilization is ${roiData.avgUtilization}. The quarterly variance is trending upwards. What are the likely drivers and what should we focus on next quarter to improve profitability?`;

    return (
        <div className="h-full flex flex-col space-y-4 animate-fade-in">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <MetricCard 
                    title="First Call Fix Rate" 
                    value={roiData.firstCallFixRate}
                    prompt="What factors are impacting our first call fix rate?"
            ...
            <Card>
                <h4 className="font-semibold text-text-primary mb-2">Profitability Variance Trend</h4>
                <div className="h-48">
                 ...
                 <button onClick={() => onOpenAiCommandCenter(prompt)} className="flex items-center justify-center gap-2 mt-4 w-full bg-component-light text-text-secondary text-sm font-bold py-2 px-3 rounded-lg transition hover:bg-border border border-border">
                    <SparklesIcon className="w-4 h-4 text-primary" /> Analyze ROI Trends
                </button>
            </Card>
        </div>
    );
